...
The task_scheduler project implements a regression
...
not broken existing functionality. The task_scheduler
...
.. code-block:: console
    ...
    (task_scheduler) $ make test

To run tests verbosely use:

.. code-block:: console

    (task_scheduler) $ make test-verbose
...
``task_scheduler`` package has been installed. If this is
not the case then you will likely need to set the ``PYTHONPATH`` environment
variable so that the ``task_scheduler`` package can be found.

.. code-block:: console

    (task_scheduler) $ cd tests
    (task_scheduler) $ python -m unittest

Individual unit tests can be run also.

.. code-block:: console

    (task_scheduler) $ python -m test_version
...
.. code-block:: console
    ...
    (task_scheduler) $ make sort-imports
...
.. code-block:: console
    ...
    (task_scheduler) $ make check-static-analysis
...
.. code-block:: console
    ...
    (task_scheduler) $ make check-lint
...
.. code-block:: console
    ...
    (task_scheduler) $ make check-types
...
.. code-block:: console
    ...
    (task_scheduler) $ make check-docs
...
.. code-block:: console
    ...
    (task_scheduler) $ make serve-docs
...
- Create and push a repo tag to Github. As a convention use the package
  ...
  - This will trigger Github to create a release at:
    ...
    ::
        ...
        https://github.com/{username}/task_scheduler/releases/{tag}
...
- Test the release distribution. This involves creating a virtual environment,
  ...
  .. code-block:: console
      ...
      (task_scheduler) $ make dist-test

- Upload the release to PyPI using

  .. code-block:: console

      (task_scheduler) $ make dist-upload

  The package should now be available at https://pypi.org/project/task_scheduler/
