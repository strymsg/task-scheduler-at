...
# help:
# help: task_scheduler Makefile help
# help:
...
venv:
	@rm -Rf venv
	@python3 -m venv venv --prompt task_scheduler
...
.PHONY: format
format:
	@black src/task_scheduler tests examples


# help: check-format                   - check code format compliance
.PHONY: check-format
check-format:
	@black --check src/task_scheduler tests examples
...
.PHONY: check-types
check-types:
	@cd src; mypy -p task_scheduler --ignore-missing-imports


# help: check-lint                     - run static analysis checks
.PHONY: check-lint
check-lint:
	@pylint --rcfile=.pylintrc task_scheduler ./tests setup.py ./examples
...
.PHONY: docs
docs: coverage
	@cd docs; rm -rf source/api/task_scheduler*.rst source/api/modules.rst build/*
	@cd docs; make html
...
.PHONY: dist-test
dist-test: dist
	@cd dist && ../tests/test-dist.bash ./task_scheduler-*-py3-none-any.whl


# help: dist-upload                    - upload a wheel distribution package
.PHONY: dist-upload
dist-upload:
	@twine upload dist/task_scheduler-*-py3-none-any.whl
